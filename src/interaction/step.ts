...
import { turnToward } from '../player/movement';
...
export function stepInteraction(runtime: PlayerRuntime, env: InteractionEnv, dt: number) {
  ...
  if (state.phase === 'free') {
    ...
    if (interact && focus) {
      if (focus.kind === 'cashier') {
        env.emit({ type: 'cashier', target: focus });
      } else {
        beginWalk(state, focus, position, Math.hypot(velocity.x, velocity.z));
        velocity.x = velocity.z = 0;
        state.focusKey = null;
        env.emit({ type: 'focus', target: null });
        emitPhase('walking', focus);
      }
    }
